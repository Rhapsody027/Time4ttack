...
from tray import create_default_icon, set_autostart, is_autostart_enabled
import pystray
...
async def start_hub_services():
    ...
    udp_transport, protocol = await loop.create_datagram_endpoint(
        lambda: FH6UDPProtocol(handle_raw_packet),
...
def setup_tray(on_exit_callback):
    ...
    menu = pystray.Menu(
        ...
        pystray.MenuItem("開機自動啟動", toggle_autostart, checked=lambda item: is_autostart_enabled()),
        pystray.Menu.SEPARATOR,
        pystray.MenuItem("關閉程式 (Exit)", lambda icon, item: on_exit_callback())
    )
    
    icon = pystray.Icon("Time4ttack FH6 Hub", create_default_icon(), "Time4ttack FH6 Telemetry Hub", menu)
    return icon
