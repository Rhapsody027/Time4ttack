...
const pedalOverlap = useTelemetryStore((state: TelemetryStoreState) => state.pedalOverlap);
...
{/* 🚀 修正：加上 whitespace-nowrap 焊死不換行，並往下修正 -bottom-6 確保絕不遮擋上方元件 */}
<div
className={`absolute -bottom-6 rounded border px-1.5 py-0.5 text-[8px] font-bold tracking-wider whitespace-nowrap ${pedalOverlap ? "animate-pulse border-yellow-500/20 bg-yellow-500/10 text-yellow-500" : "border-slate-800 bg-slate-950/80 text-slate-500"}`}
>
